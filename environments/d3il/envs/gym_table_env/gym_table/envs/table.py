...
from .objects.objects import get_obj_list, init_end_eff_pos

object_dict = get_obj_list()
...
class BlockContextManager:
    def __init__(self, scene, index=0, seed=42) -> None:
        ...
        self.banana_space = Box(
            low=np.array([0.3, -0.1, -90]), high=np.array([0.35, 0.05, 90])#, seed=seed
        )
        self.mug_space = Box(
...
class Table_Env(GymEnvWrapper):
    def __init__(
        ...
        scene = sim_factory.create_scene(
            object_list=object_dict.values(), render=render_mode, dt=0.001
        )
        robot = MjRobot(
            scene,
            xml_path=sim_framework_path("./models/mj/robot/panda_invisible.xml"),
        )
        ...
        self.banana = object_dict['banana']
        # self.apple = object_dict['apple']
        self.plate = object_dict['plate']
        self.mug = object_dict['mug']
        ...
        self.log_dict = {
            "banana": ObjectLogger(scene, self.banana),
            # "apple": ObjectLogger(scene, self.apple),
            "plate": ObjectLogger(scene, self.plate),
            "mug": ObjectLogger(scene, self.mug),
        }
